export const articleSections = [
  {
    "type": "heading",
    "level": 2,
    "content": "Optimiza tu presencia en YouTube con Generador de Descripciones"
  },
  {
    "type": "paragraph",
    "content": "SavClip te ayuda a acelerar la creación de tus metadatos con herramientas IA. Incrementa tus posibilidades de aparecer en las recomendaciones de los usuarios. Si deseas probar otras funciones, también puedes usar nuestro <a href=\"/youtube-title-generator\">Generador de Títulos</a> o consultar el <a href=\"/youtube-region-restriction-checker\">Verificador de Restricciones Regionales</a> para obtener un acceso completo."
  },
  {
    "type": "heading",
    "level": 3,
    "content": "Títulos y bios de alto impacto"
  },
  {
    "type": "paragraph",
    "content": "Analizamos las búsquedas populares del YouTube en tiempo real para recomendarte términos que se adapten a tu nicho y potencien la indexación de tus posts."
  }
];

export const faqs = [
  {
    "q": "¿Cómo generar hashtags o tags para YouTube?",
    "a": "Ingresa el tema principal o palabras clave en el cuadro de arriba, selecciona tu idioma y haz clic en generar para obtener ideas instantáneas de alto rendimiento."
  },
  {
    "q": "¿El asistente de contenido de IA es gratuito?",
    "a": "Sí, todos los generadores de hashtags, descripciones, bios y títulos son 100% gratuitos."
  },
  {
    "q": "¿Las etiquetas generadas son únicas?",
    "a": "Sí, el motor de inteligencia artificial genera sugerencias únicas basadas en el tema específico que proporciones."
  },
  {
    "q": "¿Hay un límite de uso diario para la herramienta de IA?",
    "a": "No, puedes generar descripciones, hashtags, bios y títulos todas las veces que necesites gratuitamente."
  }
];

export const meta = {
  title: "Generador de Descripciones YouTube Generar Bios y Títulos - SavClip",
  description: "Crea hashtags y descripciones para el YouTube con inteligencia artificial. Aumenta el alcance de tus posts gratis."
};

export const header = {
  title: "Generador de Descripciones",
  title1: "Crear",
  title2: "YouTube Descripciones",
  title3: "Alcance Orgánico",
  subtitle: "Optimiza el SEO de tu canal de YouTube con nuestro asistente de IA gratis."
};
